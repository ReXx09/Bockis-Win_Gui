...
# Test 5: HWiNFO Check
Write-Host "`n[5] HWiNFO Verfuegbarkeit:" -ForegroundColor Yellow
$hwinfo = Test-HWiNFOAvailable
if ($hwinfo) {
    Write-Host "    HWiNFO laeuft - Shared Memory koennte verfuegbar sein" -ForegroundColor Green
}
else {
    Write-Host "    HWiNFO nicht aktiv" -ForegroundColor Gray
}
...
$available = 0
$total = $info.Count - 1  # HWiNFO nicht mitzählen
...
Write-Host "  HWiNFO: " -NoNewline; Write-Host $(if ($info.HWiNFO) { "Aktiv" } else { "Inaktiv" }) -ForegroundColor $(if ($info.HWiNFO) { 'Green' } else { 'Gray' })
