...
Import-Module "$PSScriptRoot\FallbackSensors.psm1" -Force
...
$script:useFallbackSensors = $false  # Neue Variable für Fallback-Modus
...
function Initialize-LiveMonitoring {
    ...
    try {
        ...
        if ($null -eq $script:computerObj -or -not $script:useLibreHardware) {
            ...
            # Prüfe ob LibreHardwareMonitor ODER Fallback-Sensoren verfügbar sind
            if ($null -eq $script:computerObj -and -not $script:useFallbackSensors) {
                Write-Warning "LibreHardwareMonitor konnte nicht initialisiert werden und Fallback nicht verfügbar!"
                return $null
            }
            
            # Wenn Fallback aktiv ist, informiere darüber (nur Info, keine Warnung)
            if ($script:useFallbackSensors) {
                Write-Host "`n[ℹ] Verwende Fallback-Sensoren (Performance Counters & WMI)`n" -ForegroundColor Cyan
            }
            
            $script:useLibreHardware = ($null -ne $script:computerObj)
        ...
        $timer.Add_Tick({
                ...
                try {
                    ...
                    elseif ($script:useFallbackSensors) {
                        # Fallback-Modus (Performance Counters & WMI)
                        # CPU-Update (Last verfügbar, Temp meist nicht)
                        Update-CpuInfoFallback -CpuLabel $cpuLabel -Panel $gbCPU
                        
                        # GPU-Update (nur Name & ggf. Last)
                        if ($script:updateCounter % 2 -eq 0 -and $gpuLabel -and $gbGPU) {
                            Update-GpuInfoFallback -GpuLabel $gpuLabel -Panel $gbGPU
                        }
                        
                        # RAM-Update (Auslastung verfügbar)
                        if ($script:updateCounter % 2 -eq 1 -and $ramLabel -and $gbRAM) {
...
#region Fallback Update Funktionen

function Update-CpuInfoFallback {
    param (
        [System.Windows.Forms.Label]$CpuLabel,
    ...
    try {
        # CPU-Last via Performance Counter
        $cpuLoad = Get-CpuLoadFallback
        
        # Versuche Temperaturen via ACPI Thermal Zones (funktioniert nicht auf allen Systemen)
        $thermalZones = Get-ThermalZonesFallback
        $temp = $null
        
        if ($thermalZones -and $thermalZones.Count -gt 0) {
            # Nimm die höchste Temperatur (konservativste Annahme)
            $temp = ($thermalZones.Values | Measure-Object -Maximum).Maximum
        }
        
        # UI aktualisieren
        if ($null -ne $CpuLabel) {
            $loadText = if ($cpuLoad) { "$cpuLoad%" } else { "N/A" }
            $tempText = if ($temp) { "$([math]::Round($temp, 0))°C" } else { "N/A" }
            $CpuLabel.Text = " $loadText | $tempText | N/A | N/A"
        }
        
        # Panel-Farbe basierend auf Last (Temperatur meist nicht verfügbar)
        if ($null -ne $Panel) {
            $warningColor = if ($cpuLoad -ge 90) {
                [System.Drawing.Color]::FromArgb(255, 100, 100)  # Rot
            } elseif ($cpuLoad -ge 70) {
                [System.Drawing.Color]::FromArgb(255, 200, 100)  # Orange
            } else {
                [System.Drawing.Color]::FromArgb(100, 200, 100)  # Grün
            }
            $Panel.BackColor = $warningColor
        }
        
        # Statistiken aktualisieren
        if ($cpuLoad) { Update-HardwareStats -Component 'CPU' -Property 'Load' -Value $cpuLoad }
        if ($temp) { Update-HardwareStats -Component 'CPU' -Property 'Temp' -Value $temp }
    }
    catch {
        Write-Verbose "Fehler in Update-CpuInfoFallback: $_"
    }
}

function Update-GpuInfoFallback {
    param (
        [System.Windows.Forms.Label]$GpuLabel,
        [System.Windows.Forms.Panel]$Panel
    )
    ...
    try {
        # GPU-Name via WMI (einmalig cachen)
        if (-not $script:gpuName) {
            $gpuController = Get-WmiObject -Namespace "root\CIMV2" -Class "Win32_VideoController" -ErrorAction SilentlyContinue | 
                Where-Object { $_.Name -match "NVIDIA|AMD|Radeon|GeForce|Intel.*Graphics" } | 
                Select-Object -First 1
            
            if ($gpuController) {
                $script:gpuName = $gpuController.Name
                
                # GPU-Titel-Label setzen
                if ($Panel -and $Panel.Controls.Count -gt 0) {
                    foreach ($control in $Panel.Controls) {
                        if ($control -is [System.Windows.Forms.Label] -and $control.Location.Y -eq 0) {
                            $control.Text = "GPU: $($script:gpuName)"
                            $control.Refresh()
                            break
                        }
                    }
        ...
        # UI aktualisieren
        if ($null -ne $GpuLabel) {
            $loadText = if ($gpuLoad) { "$gpuLoad%" } else { "N/A" }
            $GpuLabel.Text = "$loadText | N/A | N/A | N/A"
        }
        
        # Panel-Farbe (Standard grau, da keine Temp verfügbar)
        if ($null -ne $Panel) {
            $Panel.BackColor = [System.Drawing.Color]::FromArgb(180, 180, 180)
        }
        
        # Statistiken
        if ($gpuLoad) { Update-HardwareStats -Component 'GPU' -Property 'Load' -Value $gpuLoad }
    }
    catch {
        Write-Verbose "Fehler in Update-GpuInfoFallback: $_"
    }
}

function Update-RamInfoFallback {
    param (
        [System.Windows.Forms.Label]$RamLabel,
        [System.Windows.Forms.Panel]$Panel
    )
    ...
    try {
        # RAM-Auslastung via CIM
        $memory = Get-MemoryUsageFallback
        
        if ($memory) {
            # UI aktualisieren
            if ($null -ne $RamLabel) {
                $RamLabel.Text = " $($memory.UsedPercent)% | N/A | $($memory.UsedGB) GB / $($memory.TotalGB) GB"
            }
            
            # Panel-Farbe basierend auf Auslastung
            if ($null -ne $Panel) {
                $warningColor = if ($memory.UsedPercent -ge 90) {
                    [System.Drawing.Color]::FromArgb(255, 100, 100)  # Rot
                } elseif ($memory.UsedPercent -ge 75) {
                    [System.Drawing.Color]::FromArgb(255, 200, 100)  # Orange
                } else {
                    [System.Drawing.Color]::FromArgb(100, 200, 100)  # Grün
                }
            ...
            # Statistiken
            Update-HardwareStats -Component 'RAM' -Property 'Load' -Value $memory.UsedPercent
            Update-HardwareStats -Component 'RAM' -Property 'Used' -Value $memory.UsedGB
        }
    }
    catch {
        Write-Verbose "Fehler in Update-RamInfoFallback: $_"
    }
}
...
function Initialize-LibreHardwareMonitor {
    ...
    Write-Host "`n🔍 Suche LibreHardwareMonitor Installation..." -ForegroundColor Cyan
    
    # PRIORITÄT 1: System-Installation (IMMER ZUERST)
    # Diese hat signierte Treiber und verursacht keine Defender-Probleme
    $systemPaths = @()
    
    # 1. WinGet-Installation (häufigster Fall)
    try {
        $wingetPaths = Resolve-Path "${env:LOCALAPPDATA}\Microsoft\WinGet\Packages\LibreHardwareMonitor*\LibreHardwareMonitorLib.dll" -ErrorAction SilentlyContinue
        if ($wingetPaths) {
            $systemPaths += $wingetPaths.Path
        }
    }
    catch {
        # Kein Winget-Paket gefunden
    }
    ...
    # PRIORITÄT 2: Integrierte Lib-Dateien (Fallback)
    $bundledPaths = @(
    ...
    # Kombiniere: System-Pfade haben Vorrang
    $possiblePaths = $systemPaths + $bundledPaths
    ...
    foreach ($path in $possiblePaths) {
        if (Test-Path $path) {
            ...
            # Prüfe ob es eine System-Installation ist (nicht in unserem Projektordner)
            $projectRoot = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
            $isSystemInstall = -not $libPath.StartsWith($projectRoot)
            
            if ($isSystemInstall) {
                Write-Host "  ✓ System-Installation gefunden: $libPath" -ForegroundColor Green
            }
            else {
                Write-Host "  ℹ️ Integrierte Lib gefunden: $libPath" -ForegroundColor Cyan
                Write-Host "    Tipp: Installiere LibreHardwareMonitor via Winget für bessere Treiber-Unterstützung" -ForegroundColor Yellow
            }
    ...
    try {
        if ($libPath -and (Test-Path $libPath)) {
            ...
            if ($cpuHardware) {
                ...
                Write-Host "  ✓ Hardware-Monitor initialisiert" -ForegroundColor Green
                $script:useLibreHardware = $true
            }
            ...
            Write-Warning "LibreHardwareMonitorLib.dll nicht gefunden"
            Write-Host "`n💡 Für volle Hardware-Sensor-Unterstützung installiere LibreHardwareMonitor:" -ForegroundColor Yellow
            Write-Host "   winget install LibreHardwareMonitor.LibreHardwareMonitor`n" -ForegroundColor Cyan
            Write-Host "  ℹ️ Wechsle zu Fallback-Sensoren (Performance Counters & WMI)" -ForegroundColor Cyan
            $script:useFallbackSensors = $true
            return $null
        }
    }
...
function Initialize-HardwareMonitoring {
    ...
    if (-not $timerStatus.Exists) {
        ...
        foreach ($component in $hardwareComponents) {
            ...
            switch ($component.Name) {
                "LibreHardwareMonitor-Bibliothek" {
                    ...
                    if ($script:useFallbackSensors) {
                        Write-Host "`n[ℹ] Verwende Fallback-Sensoren (Performance Counters & WMI)" -ForegroundColor Cyan
                    }
                    else {
                        $script:useLibreHardware = $true
                    }
        ...
        $timerStatus = Get-HardwareTimerStatus
        if (-not $timerStatus.Exists) {
            # Im Fallback-Modus ist kein Timer nötig
            if ($script:useFallbackSensors) {
                if (-not $SuppressVisualFeedback) {
                    Write-Host "`r`t[ℹ] Fallback-Modus aktiv - kein Hardware-Timer erforderlich" -ForegroundColor Cyan
                }
                return $true
            }
